...
### 2026年7月反映の改定（レベル4関連）
- レベル4（腹腔鏡を用いた手術等が行われる場合に相当する区分）の基本点数を、イ 9,130→9,015点、ロ 6,610→6,500点に調整
- レベル4の時間延長加算を660→650点に調整
- 当該改定は名称変更を伴う一連の見直しの一部だが、アプリは元々区分の正式名称を表示していないため表示ラベルへの変更はなし
- 上記以外のレベル・硬膜外麻酔加算・神経ブロック加算・時間外/深夜休日加算の倍率は変更なし
